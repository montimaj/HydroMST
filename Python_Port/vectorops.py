...
def shp2raster(input_shp_file, outfile_path, value_field='GW_Pumping', xres=1000., yres=1000.,
    ...
    :param value_field: Value field to use during conversion
    ...
    layer_name = input_shp_file[input_shp_file.rfind('/') + 1: input_shp_file.rfind('.')]
...
def csvs2shps(input_dir, output_dir, outfile_prefix='GW_', pattern='*.csv', target_crs='EPSG:4326', delim=','):
    """
    ...
    for file in glob(input_dir + pattern):
        outfile_path = output_dir + outfile_prefix + file[file.rfind('/') + 1: file.rfind('.') + 1] + 'shp'
        csv2shp(file, outfile_path=outfile_path, delim=delim, target_crs=target_crs)


def shps2rasters(input_dir, output_dir, value_field='GW_Pumping', xres=1000, yres=1000,
               gdal_rasterize_path='/usr/local/Cellar/gdal/2.4.2/bin/gdal_rasterize'):
    """
