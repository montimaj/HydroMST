...
# this code uses a subsidence model to solve for hydrologic parameters.
# The subsidence model models land subsidence with a known hydraulic head history. 
...
def subsidence_model(x):
    ...
    # This part determines the residual (difference) between modeled and observed subsidence.
    # Here I just have the total subsidence value from 2015 to 2020, but it would be
    # More accurate to include for the residual all of the available InSAR data, then take the 
    # root mean square of the difference (pseudo code below):
    # interp_model_data = np.interp(insar_time,time,ts_subs[:,1])
    # diff = insar_data-interp_model_data
    # residual=np.sqrut(np.mean(np.square(diff)))
    time=ts_subs[:,0]/365+time_bound[0]
    min_ind=np.argmin(np.abs(time-2015))
    print(time[min_ind])
    modeled_subs_since2015=ts_subs[-1,1]-ts_subs[min_ind,1]
    true_subs_since2015=.15*5
    residual=np.abs(modeled_subs_since2015-true_subs_since2015)
    print('residual: '+str(residual))
    return(residual)
    
from scipy.optimize import minimize

parameters=minimize(subsidence_model,(-8,nclay),method='Nelder-Mead')
